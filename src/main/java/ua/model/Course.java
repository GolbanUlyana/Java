...
public class Course {
    ...
    public Course(String name, String description, int credits, 
        ...
        validateData(name, description, credits, startDate, level);
        this.name = name.trim();
    ...
    // Конструктор з InvalidDataException
    public static Course createCourseWithValidation(String name, String description, int credits, 
                                                   LocalDate startDate, CourseLevel level) 
        ...
        try {
            ...
            throw new InvalidDataException("Некоректні дані курсу: " + e.getMessage(), e);
        }
    }
    
    // Factory метод (без винятків)
    ...
    // Валідація
    private static void validateData(String name, String description, int credits, 
                                   LocalDate startDate, CourseLevel level) {
